...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:hsqueue="xalan://com.healthsolve.xsl.MessageQueue" xmlns:hshtml="xalan://com.healthsolve.xsl.HTML2FO">
  ...
  <xsl:variable name="patient-appointments-query-string">
    <xsl:if test="/request/page != ''">
      <xsl:text>current_appointments[page]=</xsl:text><xsl:value-of select="/request/page"/>
    </xsl:if>

    <xsl:if test="/request/from_date != ''">
      <xsl:text>&amp;current_appointments[from_date]=</xsl:text><xsl:value-of select="/request/from_date"/>
    </xsl:if>

    <xsl:if test="/request/to_date != ''">
      <xsl:text>&amp;current_appointments[to_date]=</xsl:text><xsl:value-of select="/request/to_date"/>
    </xsl:if>

    <xsl:if test="/request/provider != ''">
      <xsl:text>&amp;current_appointments[provider]=</xsl:text><xsl:value-of select="/request/provider"/>
    </xsl:if>

    <xsl:if test="/request/location != ''">
      <xsl:text>&amp;current_appointments[location]=</xsl:text><xsl:value-of select="/request/location"/>
    </xsl:if>

    <xsl:if test="/request/appointment_types != ''">
      <xsl:text>&amp;current_appointments[appointment_types]=</xsl:text><xsl:value-of select="/request/appointment_types"/>
    </xsl:if>

    <xsl:if test="/request/appointment_status != ''">
      <xsl:text>&amp;current_appointments[appointment_status]=</xsl:text><xsl:value-of select="/request/appointment_status"/>
    </xsl:if>
  </xsl:variable>

  <xsl:variable name="current_appointments" select="document(concat(
    '/patients/',
    /request/patient_id,
    '/appointments.xml?',
    $patient-appointments-query-string
  ))"/>
  ...
  <xsl:template name="layout-header">
    <fo:block text-align="center" width="297mm">
      <fo:external-graphic src="system_images/HeaderL" content-height="30mm" />
    </fo:block>
